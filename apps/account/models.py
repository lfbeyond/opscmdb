...
class Structure(MPTTModel):
    ...
    arent = TreeForeignKey('self', on_delete=models.SET_NULL, verbose_name='上级业务', null=True, blank=True,db_index=True ,related_name='children')
    ...
    def node_path(self):
        ...
        if not self.parent:
            ...
            dataList = Structure.objects.raw("""SELECT id,text as path FROM apulis_structure WHERE tree_id = {tree_id} AND  lft < {lft} AND  rght > {rght} ORDER BY lft DESC;""".format(tree_id=self.tree_id,lft=self.lft,rght=self.rght))
            for ds in dataList:
